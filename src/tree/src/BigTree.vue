...
<template>
  <div ref="scrollerRef" :class="prefixCls" :style="wrapStyle" @scroll="handleScroll">
      ...
      <div :class="`${prefixCls}-content`" :style="{ transform: `translateY(${offset}px)` }">
        ...
        >
          <div :class="arrowClasses(node)" @click="handleExpand(node)">
            <i
              v-if="node[childrenKey] && node[childrenKey].length"
          ...
          <b-checkbox
            ...
            @click.prevent="handleCheck(node)"
          ></b-checkbox>
          <span
            v-if="node.display"
            :class="titleClasses(node)"
            @click="handleSelect(node)"
            v-html="node.display"
          ></span>
          <span v-else :class="titleClasses(node)" @click="handleSelect(node)">
            {{ node[titleKey] }}
          </span>
        </div>
      </div>
...
import { bigTreeProps } from './types'
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    function getContentHeight() {
      states.contentHeight =
        // @ts-ignore
        (states.flattenTree || []).filter(item => item.visible).length * props.itemHeight + 'px'
    }
