...
树形表格使用 `children` 作为子节点字段，配合 `row-key` 与 `expand-column-key` 开启。
...
通过设置属性 `size` 为 `large` 或 `small` 可以调整表格尺寸。默认行高为 `40px`，`small` 为 `36px`，默认不填或填写 `default`。
...
| default-expanded-row-keys | 树形表格默认展开的行 keys                                                                                | Array         | —             | []       |
| expanded-row-keys    | 树形表格受控展开的行 keys                                                                                     | Array         | —             | —        |
...
| update:expandedRowKeys | 树形表格受控展开 keys 更新时触发                        | expandedRowKeys                                                |
