...
`height` 和 `max-height` 都可以设置固定表头，表格主体区域会通过内置 scrollbar 滚动，固定列会同步滚动位置。
...
树形表格固定使用 `children` 作为子节点字段，需同时传入字符串类型的 `row-key` 和
`expand-column-key` 才会开启。

支持：
- `default-expanded-row-keys` 设置默认展开项
- `expanded-row-keys` + `@update:expanded-row-keys` 进行受控展开
- 排序时按同级节点分别排序，保持树层级不被打平
- 与固定列、固定高度一起使用

当前限制：
- 不支持与 `type: 'expand'` 同时使用
- 不支持与 `draggable` 同时使用
- 不建议与 `mergeMethod` 同时使用
...
通过设置属性 `size` 为 `large`、`default` 或 `small` 可以调整表格尺寸。默认行高为 `40px`，`small` 为 `36px`，不传时等同于 `default`。
...
| height               | 表格高度，单位 px。设置后表格主体通过内置 scrollbar 滚动，并固定表头                                          | Number/String | —             | —        |
| max-height           | 表格最大高度，单位 px。超出后表格主体通过内置 scrollbar 滚动，并固定表头                                      | Number/String | —             | —        |
...
| tooltip-theme        | 开启列 `tooltip` 时 Tooltip 主题                                                                               | String        | dark / light  | —        |
| row-key              | 是否强制使用内置 row-key 刷新；树形表格模式下传入业务主键字段名                                              | Boolean/String | —            | false    |
| expand-column-key    | 树形表格展开列对应的 key，设置后且 `row-key` 为字符串时开启树形表格                                           | String        | —             | ''       |
| default-expanded-row-keys | 树形表格默认展开的行 keys，非受控模式下生效                                                              | Array         | —             | []       |
| expanded-row-keys    | 树形表格受控展开的行 keys，配合 `update:expandedRowKeys` 使用                                                 | Array         | —             | —        |
| indent-size          | 树形表格缩进宽度                                                                                              | Number        | —             | 16       |
| merge-method         | 表格合并行列的方法函数                                                                                        | Function      | —             | false    |
| edit-table           | 开启编辑表格样式                                                                                              | Boolean       | false/true    | false    |
| edit-table-detail    | 在 `edit-table` 基础上开启更紧凑的明细态编辑样式                                                              | Boolean       | false/true    | false    |
...
| expand            | `type: 'expand'` 展开行展开或收起时触发                      | row：当前行的数据，status：当前状态                            |
| update:expandedRowKeys | 树形表格受控展开 keys 更新时触发，模板监听请使用 `@update:expanded-row-keys` | expandedRowKeys                                |
| expand-change     | 树形表格展开状态变化时触发                                  | row, expanded, expandedRowKeys                                 |
...
| clearCurrentRow | 清除高亮项，仅在开启 `highlight-row` 时有效                          | 无    |
| handleResize    | 重新计算列宽、固定列和 scrollbar 尺寸；容器显隐或布局变化后可手动调用 | 无    |
| getSelection    | 获取已经选中的行                                                     | 无    |
| selectAll       | 设置当前可选行的全选 / 取消全选状态                                  | status |
...
| slot        | 使用具名插槽渲染当前列，插槽参数为 `row`、`column`、`index`       | String            | -                                     | -      |
| render      | 自定义渲染列 ,第一个是 h，第二个为对象，包含 row、column 和 index | Function          | -                                     | -      |
| renderHeader | 自定义表头渲染，参数为 `{ column, index }`                        | Function          | -                                     | -      |
...
| children    | 表头分组子列配置；父列的 fixed 会继承给子列                        | Array             | -                                     | -      |
